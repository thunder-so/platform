...
DROP POLICY IF EXISTS "View user profiles via membership or self" ON users;
DROP POLICY IF EXISTS "View orders in org" ON orders;
DROP POLICY IF EXISTS "Manage orders as owner/admin" ON orders;
...
-- Create helper view: user's active organizations
DROP VIEW IF EXISTS v_user_organizations;
...
-- Users: Can only access their own record or users in same org
CREATE POLICY "View user profiles via membership or self" ON users
  FOR SELECT USING (
    auth.uid() = id
    OR id IN (
      SELECT m.user_id
...
-- Memberships: View all members in orgs user belongs to
CREATE POLICY "View organization members" ON memberships
  FOR SELECT USING (
    deleted_at IS NULL
