<template>
  <ClientOnly>
  <UForm ref="form" v-if="service && service.stack_type === 'LAMBDA'" :state="service" :validate-on="['input']" class="space-y-6">
      ...
      <UFormField v-if="deploymentMode === 'Container'" label="Docker File" description="The path to your Dockerfile." name="metadata.functionProps.dockerFile" class="grid grid-cols-3 gap-4">
        <UInput v-model="(service.metadata as any).functionProps.dockerFile" placeholder="Dockerfile" class="w-96" size="lg" />
      </UFormField>
      ...
      <UFormField v-if="deploymentMode === 'Zip'" label="Runtime" description="Runtime for the function." name="metadata.functionProps.runtime" class="grid grid-cols-3 gap-4">
        <USelect v-model="(service.metadata as any).functionProps.runtime" :items="lambdaRuntimes" class="w-96" size="lg" />
      </UFormField>
      <UFormField v-if="deploymentMode === 'Zip'" label="Code Directory" description="Directory containing built code (e.g. dist/)." name="metadata.functionProps.codeDir" class="grid grid-cols-3 gap-4">
        <UInput v-model="(service.metadata as any).functionProps.codeDir" placeholder="dist/" class="w-96" size="lg" required />
      </UFormField>
      <UFormField v-if="deploymentMode === 'Zip'" label="Handler" description="Function handler (e.g. index.handler)." name="metadata.functionProps.handler" class="grid grid-cols-3 gap-4">
        <UInput v-model="(service.metadata as any).functionProps.handler" placeholder="index.handler" class="w-96" size="lg" />
      </UFormField>
      <UFormField label="Memory Size (MB)" description="The memory allocated to your Lambda." name="metadata.functionProps.memorySize" class="grid grid-cols-3 gap-4">
        <UInputNumber v-model="(service.metadata as any).functionProps.memorySize" :default-value="1792" :min="128" :max="10240" size="lg" />
      </UFormField>
      <UFormField label="Keep Warm" description="Keep the Lambda warm by invoking every 5 minutes." name="metadata.functionProps.keepWarm" class="grid grid-cols-3 gap-4">
        <USwitch v-model="(service.metadata as any).functionProps.keepWarm" />
      </UFormField>
    </UForm>
  </ClientOnly>
</template>
...
// Type guard to ensure we have Lambda metadata
if (props.service.stack_type === 'LAMBDA' && !(props.service.metadata as any).functionProps) {
  (props.service.metadata as any).functionProps = {} as any;
}
